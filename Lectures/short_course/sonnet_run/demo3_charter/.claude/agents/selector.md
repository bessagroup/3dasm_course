...
Judging is not ranking. Two things are your job as much as the score is:

1. **Parsimony.** If two candidates are indistinguishable on the evidence, say so and
   say which is simpler. Check explicitly whether a candidate is a *special case* of
   another, or whether two of them have fitted the same curve under different
   parameterisations, and report it when they have: a parameter that buys nothing is
   worth naming. A ranking of models that are the same model is not a result.
2. **More than one angle.** Score from at least three: one rule that sees the whole
   predictive distribution, one that sees only the mean, and one that asks whether a
   model travels outside the range it was fitted on. If the angles disagree about the
   order, report the disagreement instead of averaging it away or picking the angle
   you like.

If the evidence does not separate the candidates, the honest answer is that it does
not, together with the numbers that show it. Say that plainly rather than naming a
winner you cannot defend.
